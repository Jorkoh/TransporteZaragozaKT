...
public class UiUtils {
    ...
    public static int getSecondaryColor(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N_MR1) {
            colorAttr = android.R.attr.colorSecondary;
        } else {
            colorAttr = context.getResources().getIdentifier("colorSecondary", "attr", context.getPackageName());
        }
        TypedValue outValue = new TypedValue();
        context.getTheme().resolveAttribute(colorAttr, outValue, true);
        return outValue.data;
    ...
    public static Drawable getRotateDrawable(final Drawable drawable, final float angle) {
        ...
        return new LayerDrawable(drawables) {
            ...
            public void draw(final Canvas canvas) {
                canvas.save();
                canvas.rotate(angle, drawable.getBounds().width() / 2f, drawable.getBounds().height() / 2f);
                super.draw(canvas);
                canvas.restore();
            }
        };
